...
int getNumbersOfLeftBranch(tree* root, int numbersOfLeftBranch);
...
int main()
{
	...
	while (true)
	{
		...
		if (checkInt(number, userInput))
		{
			...
			case 7:
			{
				cout << "Количество левых дочерних вершин: " << getNumbersOfLeftBranch(root, 0) << endl;
				break;
			}
			case 0:
				exit(0);
...
void printMenu()
{
	...
	cout << "7 - Вывод количества левых дочерних элементов\n";
...
int getNumbersOfLeftBranch(tree* root, int numbersOfLeftBranch)
{
	...
	else
	{
		...
		{
			numbersOfLeftBranch = getNumbersOfLeftBranch(root->right, numbersOfLeftBranch);
		}
		if (root->left != nullptr)
		{
			numbersOfLeftBranch = getNumbersOfLeftBranch(root->left, numbersOfLeftBranch + 1);
		}
	}
	return numbersOfLeftBranch;
}
